output "server_info" {
  value = {
    ...
    "ssh_cmd_example_pub" = try(format("ssh sacloud-user@%s -p %s -i [ssh_key]", sakuracloud_vpc_router.vpcrt[0].public_ip, var.vpcrt["pf_ssh_pub"]), ""),
    "ssh_cmd_example_prv" = format("ssh sacloud-user@%s -i [ssh_key]", sakuracloud_server.zbx_sv[0].ip_address)
  }
}
